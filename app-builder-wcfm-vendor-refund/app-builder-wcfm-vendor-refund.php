...
/**
 * Plugin Name: App Builder - Vendor Refund
 * Plugin URI: https://appcheap.io/docs
 * Text Domain: app_builder_wcfm_vendor_refund
 * Domain Path: /languages/
 * Description: Support Vendor Refund
 * Author: Appcheap
 * Version: 4.3.2
...
function app_builder_wcfm_vendor_refund_domain()
{
    load_plugin_textdomain('app_builder_wcfm_vendor_refund', false, dirname(plugin_basename(__FILE__)) . '/languages/');
}

add_action('plugins_loaded', 'app_builder_wcfm_vendor_refund_domain');

function create_refund_request($request)
{
	...
	$getBody = $request->get_body();
	error_log(print_r($getBody, true));
	// echo $reason;
	if (isset($reason) && !empty($reason)) {

		$refund_reason          = strip_tags(wcfm_stripe_newline($reason));
		$refund_reason          = wp_filter_post_kses(wp_unslash($refund_reason));
		$order_id               = absint($request->get_param('order_id'));
		$refund_request         = wc_clean($request->get_param('request'));
		$wcfm_refund_inputs     = wc_clean($request->get_param('inputs'));
		$tax_inputs             = $request->get_param('tax_inputs');
		$wcfm_refund_tax_inputs = isset($tax_inputs) ? wc_clean($tax_inputs) : array();
		...
		foreach ($wcfm_refund_inputs as $wcfm_refund_input_id => $wcfm_refund_input) {
			...
			error_log(print_r($refund_item_id, true));
			...
			if ($refund_request == 'full') {
				...
				if ((float)$refunded_amount > ((float)$item_total - (float)$old_refunded_amount)) {
					return new WP_Error('refund_request_amount_item_value', __('Refund request amount more than item value.', 'wc-multivendor-marketplace'), array('status' => 401));
				}
				...
				if (wc_tax_enabled()) {
					...
					if ((float)$refunded_tax_amt > (float)$actual_tax_amount) {
						return new WP_Error('refund_request_tax_amount_item_value', __('Refund request tax amount more than item actual tax value.', 'wc-multivendor-marketplace'), array('status' => 401));
					}
		...
		return new WP_Error('no_refund_reason', $wcfm_refund_messages['no_refund_reason'], array('status' => 404));
	}

	if (!$refund_request_processed) {
		return new WP_Error('no_item_selected_refund_request', __('No item selected for refund request.', 'wc-multivendor-marketplace'), array('status' => 401));
	}
	return rest_ensure_response(array(
		'status'    => true,
		'message'   => __('Refund requests successfully processed.', 'wc-multivendor-marketplace'),
	));
}
...
add_action('rest_api_init', 'register_wcfm_refund_endpoints');